...
const STORAGE_KEY_STATS = 'nerd_buddy_streak_stats';
const STORAGE_KEY_BADGES = 'nerd_buddy_unlocked_badges';
...
export class GamificationService {
  ...
  private async loadFromStorage(): Promise<void> {
    if (typeof chrome !== 'undefined' && chrome.storage?.local) {
      chrome.storage.local.get([STORAGE_KEY_STATS, STORAGE_KEY_BADGES], (res) => {
        if (res[STORAGE_KEY_STATS]) {
          this.stats = { ...this.stats, ...res[STORAGE_KEY_STATS] };
        } else {
          // Initialize mock past 30 days data so user has visual squares on day 1
          this.initSampleHeatmap();
        }

        if (res[STORAGE_KEY_BADGES]) {
          const loadedBadges = res[STORAGE_KEY_BADGES] as Record<BadgeId, Badge>;
